...
def get_just_gym_hours(link_key, headers):
    try:
        justgym_hours = {day: "00:00-23:59" for day in ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"]}
        days = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"]

        res = requests.get(link_key, headers=headers)
        soup = BeautifulSoup(res.text, 'html.parser')
        all_hours = soup.find_all('div', class_='hours-row')
        for day, row in zip(days, all_hours):
            hour_text = row.find('span').text.strip()
            justgym_hours[day] = hour_text
        return justgym_hours
    except Exception as e:
    ...
    try:
        ...
        for item in clubs_items:
            ...
            if item.get('data-map-twentyfour') == '':
                ...
                hours = get_just_gym_hours(item.get('data-map-link'), headers)
            else:
                hours = justgym_hours

            gym_row = {
                "id": f"justgym-{item.get('data-map-city')}-{item.get('data-map-address')}".replace(" ", "-").lower(),
